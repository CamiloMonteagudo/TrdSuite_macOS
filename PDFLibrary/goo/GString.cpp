...
//Introducido para hacer mas eficiente el trabajo con cadenas largas
  if (len<256)
    delta = 63; //7;
  else if (len<1024)
    delta = 1023;
...
inline void GString::resize(int length1) {
  ...
  if (!s) {
    s = new char[size(length1)];
		...
		if (reservedSize > 0 && size(length1) <= reservedSize) {
			//length = length1;
		} 
...
void GString::reservSize(int length1) {
	int aLength = length;

	resize( length1);
	length = aLength;
	reservedSize = size(length1);
}
...
GString::GString(int reservedSizeA) {
  s = NULL;
	length = 0;
	reservedSize = 0;
  reservSize(reservedSizeA);
  s[0] = '\0';
}
