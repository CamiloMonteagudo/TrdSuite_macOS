...
CharCodeToUnicode *CharCodeToUnicode::parseCIDToUnicode(GString *fileName,
  ...
  if (!(f = fopen(fileName->getCString(), "r"))) {
    error(-1, S_OPEN_CIDTOUNICODE_FILE,
	  fileName->getCString());
    return NULL;
  }
  ...
  while (getLine(buf, sizeof(buf), f)) {
    ...
    if (sscanf(buf, "%x", &u) == 1) {
      ...
      error(-1, S_BAD_LINE_CIDTOUNICODE_FILE,
...
CharCodeToUnicode *CharCodeToUnicode::parseUnicodeToUnicode(
  ...
  if (!(f = fopen(fileName->getCString(), "r"))) {
    error(-1, S_OPEN_UNICODETOUNICODE_FILE,
	  fileName->getCString());
    return NULL;
  }
  ...
  while (getLine(buf, sizeof(buf), f)) {
    ...
    if (!(tok = strtok(buf, " \t\r\n")) ||
	sscanf(tok, "%x", &u0) != 1) {
      error(-1, S_BAD_LINE_UNICODETOUNICODE_FILE,
...
void CharCodeToUnicode::parseCMap1(int (*getCharFunc)(void *), void *data,
  ...
  while (pst->getToken(tok2, sizeof(tok2), &n2)) {
	...
	if (!pst->getToken(tok2, sizeof(tok2), &n2) ||
	  ...
	  error(-1, S_BFRANGE_UNICODE_CMAP);
	  break;
	}
	if (!(n1 == 2 + nDigits && tok1[0] == '<' && tok1[n1 - 1] == '>' &&
...
void CharCodeToUnicode::addMapping(CharCode code, char *uStr, int n,
  ...
  if (n <= 4) {
    if (sscanf(uStr, "%x", &u) != 1) {
      error(-1, S_ILLEGAL_UNICODE_CMAP);
      return;
    }
    map[code] = u + offset;
  } else {
