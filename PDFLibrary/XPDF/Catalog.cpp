...
Catalog::Catalog(XRef *xrefA) {
  ...
  if (!catDict.isDict()) {
    error(-1, S_CATALOG_WRONG_TYPE, catDict.getTypeName());
    goto err1;
  }
  ...
  if (!obj.isNum()) {
    error(-1, S_PAGE_COUNT_WRONG_TYPE,
	  obj.getTypeName());
    goto err3;
  }
  ...
  if (numPages != numPages0) {
    error(-1, S_PAGE_COUNT_INCORRECT);
  }
  pagesDict.free();
...
GString *Catalog::readMetadata() {
  ...
  if (!dict->lookup("Subtype", &obj)->isName("XML")) {
    error(-1, S_UNKNOWN_METADATA_TYPE,
